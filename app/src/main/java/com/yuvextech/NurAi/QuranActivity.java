...
public class QuranActivity extends AppCompatActivity {
	...
	private String sorahIndex = "";
	private HashMap<String, Object> hashMap = new HashMap<>();
	private String id = "";
	private String idSurah = "";
	private int idSurahInt;
	...
	private ArrayList<HashMap<String, Object>> listState = new ArrayList<>();
	...
	private void initialize(Bundle _savedInstanceState) {
		...
		options.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View _view) {
				if (withTranslate) {
					...
					SettingsBottomdialogFragmentActivity.show(getSupportFragmentManager(), SettingsBottomdialogFragmentActivity.getTag());
				} else {
					// --- NON-TRANSLATION MODE ---
					SettingsBottomdialogFragmentActivity SettingsBottomdialogFragmentActivity = new SettingsBottomdialogFragmentActivity();
					SettingsBottomdialogFragmentActivity.show(getSupportFragmentManager(), SettingsBottomdialogFragmentActivity.getTag());
		...
		listview1.setOnItemClickListener(new AdapterView.OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> _param1, View _param2, int _param3, long _param4) {
				...
				// 1. Crear un Bundle para los datos
				Bundle zabi = new Bundle();
				zabi.putString("Aya", quranListMap.get((int)_position).get("text").toString().concat("﴿".concat(String.valueOf((long)(Double.parseDouble(quranListMap.get((int)_position).get("id").toString()))).concat("﴾"))));
				zabi.putString("Translation", quranListMap.get((int)_position).get("translation").toString());
				zabi.putString("Transliterate", translations);
				zabi.putString("surah", idSurah);
				zabi.putString("AyaId", quranListMap.get((int)_position).get("id").toString());
				InfoBottomdialogFragmentActivity InfoBottomdialogFragmentActivity = new InfoBottomdialogFragmentActivity();
				InfoBottomdialogFragmentActivity.setArguments(zabi);
				InfoBottomdialogFragmentActivity.show(getSupportFragmentManager(), "dialogTag");
			}
		});
	...
	private void initializeLogic() {
		withTranslate = true;
		_Ui();
		if (!sh.getString(getIntent().getStringExtra("sorah"), "").equals("")) {
			listState.clear();
			listState = new Gson().fromJson(sh.getString(getIntent().getStringExtra("sorah"), ""), new TypeToken<ArrayList<HashMap<String, Object>>>(){}.getType());
			try { 
				listview1.setSelectionFromTop((int)Double.parseDouble(listState.get((int)0).get("index").toString()), (int)Double.parseDouble(listState.get((int)0).get("top").toString()));
			} catch (Exception e){
				e.printStackTrace();
			}
		} else {
	...
	public void onStop() {
		...
		listState.clear();
		hashMap = new HashMap<>();
		hashMap.put("index", String.valueOf((long)(listview1.getFirstVisiblePosition())));
		hashMap.put("top", String.valueOf((long)((listview1.getChildAt(0) == null) ? 0 : (listview1.getChildAt(0).getTop() - listview1.getPaddingTop()))));
		hashMap.put("chapter", getIntent().getStringExtra("sorah"));
		listState.add(hashMap);
		sh.edit().putString(getIntent().getStringExtra("sorah"), new Gson().toJson(listState)).commit();
	}
	public void _Ui() {
		toolBarText.setTextColor(getResources().getColor(R.color.text_on_primary));
		toolBarSubText.setTextColor(getResources().getColor(R.color.text_on_primary));
		//  Get the intent data FIRST
		sorahIndex = getIntent().getStringExtra("sorah");
		
		// Safety check: if the intent is missing data, don't proceed
		if (sorahIndex == null || sorahIndex.isEmpty()) {
			Log.e("QuranActivity", "Sorah index is missing!");
			return; 
		}
		try {
			...
			_get(Double.parseDouble(sorahIndex));
			surah = new Gson().fromJson(jsonarray.toString(), new TypeToken<ArrayList<HashMap<String, Object>>>(){}.getType());
			names = surah.get((int)Double.parseDouble(sorahIndex)).get("name").toString();
			translations = surah.get((int)Double.parseDouble(sorahIndex)).get("translation").toString();
			totally = surah.get((int)Double.parseDouble(sorahIndex)).get("total_verses").toString();
			id = surah.get((int)Double.parseDouble(sorahIndex)).get("id").toString();
			idSurah = String.valueOf((long)(Double.parseDouble(id)));
			idSurahInt = Integer.parseInt(idSurah);
		} catch(Exception e) {
		}
		if (withTranslate) {
	...
	public class Listview1Adapter extends BaseAdapter {
		...
		public View getView(final int _position, View _v, ViewGroup _container) {
			...
			surahName.setVisibility(View.GONE);
			...
			if (_position == 0) {
				surahName.setVisibility(View.VISIBLE);
				bismillah.setVisibility(View.VISIBLE);
			} else {
				surahName.setVisibility(View.GONE);
				bismillah.setVisibility(View.GONE);
			}
			if ((_position == 0) && id.equals("9.0")) {
				bismillah.setVisibility(View.GONE);
			}
			...
			surahName.setText(String.format("surah%03d", idSurahInt).concat("  surah-icon"));
			surahName.setTypeface(Typeface.createFromAsset(getAssets(),"fonts/surah_name_v4.ttf"), 0);
